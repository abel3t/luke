...
pub fn build(b: *std.Build) void {
    ...
    // 1. Link standard C library
    exe.root_module.link_libc = true;

    // 2. Add Include Paths so Zig can find the C headers
    ...
    // 3. Compile Tree-Sitter Core (lib.c)
    exe.root_module.addCSourceFile(.{
        .file = b.path("vendor/tree-sitter/lib/src/lib.c"),
        .flags = &[_][]const u8{ "-std=c11" },
    });

    // 4. Compile the TypeScript Grammar Parser & Scanner
    exe.root_module.addCSourceFile(.{
    ...
    // 5. Compile the TSX Grammar Parser & Scanner
    exe.root_module.addCSourceFile(.{
    ...
    // 6. Compile the Go Grammar Parser
    exe.root_module.addCSourceFile(.{
        .file = b.path("vendor/tree-sitter-go/src/parser.c"),
        .flags = &[_][]const u8{ "-std=c11" },
    });
